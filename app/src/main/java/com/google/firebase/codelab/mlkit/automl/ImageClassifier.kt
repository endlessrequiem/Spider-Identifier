...
internal constructor(context: Context) {
  ...
  private val printTopKLabels: (List<FirebaseVisionImageLabel>) -> String = {
    it.joinToString(
      ...
      String.format(Locale.getDefault(), "I see a %s \nConfidence: %4.2f", label.text,
        label
          .confidence)
    }
  }
  ...
  companion object {
    ...
    private const val LOCAL_MODEL_NAME =
      "automl/model.tflite"

    /** Path of local model file stored in Assets.  */
    private const val LOCAL_MODEL_PATH = "automl/spider_metadata.json"

    /** Name of the remote model in Firebase ML Kit server.  */
    private const val REMOTE_MODEL_NAME = "spidersv1"
